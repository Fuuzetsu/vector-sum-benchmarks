...
module Benchmarking.VectorSum.UncheckedStFromBackBailEmpty (vsum) where
...
-- Vectors are traversed from the back. Simply returns first vector if
-- it's the only input, saving on ST operations when unnecessary.
